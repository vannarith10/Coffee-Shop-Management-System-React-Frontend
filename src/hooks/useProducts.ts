...
export function useProducts(): UseProductsResult {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const backendError =
        err instanceof ProductApiError ? err.backendError : toBackendError(err);
      ...
      if (backendError.status === 401 && !isRefreshing) {
        ...
        if (!ok) {
          try {
            ...
            if (!cancelled) {
              const normalized = Array.isArray(data2)
                ? data2.map(normalizeProduct)
                : [];
              setProducts(normalized);
              return;
            }
            ...
            if (!cancelled) {
              setError(
                retryErr instanceof ProductApiError
                  ? retryErr.backendError
                  : toBackendError(retryErr),
              );
            }
            return;
          }
