...
function FilterChip({ label, count, active = false, onClick }: FilterChipProps) {
  return (
    <button
      onClick={onClick}
      className={`px-4 py-2 border rounded-full flex items-center gap-2 cursor-pointer transition-colors whitespace-nowrap ${
        active
...
export default function StaffFilterChips({ active, counts, onChange }: StaffFilterChipsProps) {
  ...
  return (
    <section className="py-4 flex flex-wrap gap-2 md:gap-4 flex-shrink-0">
      {options.map((opt) => (
        <FilterChip
          key={opt.value}
          label={opt.label}
